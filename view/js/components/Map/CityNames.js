...
export class CityNames extends Map {
    renderTile(tile) {
        ...
        const { x, y } = tile, size = this.tileSize(), offsetX = x * size, offsetY = y * size, city = tile.city, sizeOffsetX = this.tileSize() / 2, sizeOffsetY = this.tileSize() * 0.75, textOffsetX = this.tileSize() / 2, textOffsetY = this.tileSize() * 1.6;
