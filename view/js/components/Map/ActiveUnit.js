...
export class ActiveUnit extends Units {
    render() {
        ...
        if (Object.values(tile.units).length > 0) {
            const player = activeUnit.player, civilization = player.civilization, [colors] = Object.values(civilization.attributes).filter((attribute) => attribute.name === 'colors'), image = this.replaceColors(this.getPreloadedImage(`units/${activeUnit._.toLowerCase()}`), 
