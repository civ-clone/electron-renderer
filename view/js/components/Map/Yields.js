...
export class Yields extends Map {
    renderTile(tile) {
        ...
        const { x, y } = tile, size = this.tileSize(), offsetX = x * size, offsetY = y * size, total = tile.yields.reduce((total, tileYield) => total + tileYield.value, 0), 
        // TODO: use a more effective sort than this, this only works by luck (Food -> Production -> Trade)
        yields = tile.yields.sort((a, b) => a._.charCodeAt(0) - b._.charCodeAt(0));
        let i = 0;
        if (total < 5) {
            const offsets = [
                [offsetX, offsetY],
